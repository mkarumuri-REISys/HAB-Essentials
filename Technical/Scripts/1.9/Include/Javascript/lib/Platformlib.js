        ...
        .click(function () {
            $.ajax({
                ...
                success: function (data) {
					ReiSys.Utilities.Util.PostJsonToServiceWithNoParams(ReiSys.Utilities.Util.BaseUrl + "Platform/Interface/Services/SessionService.aspx" + '/UpdateCallOut');
...
function ListBoxOnLoad(sender) {
	// Fix for "Input String is not in correct format" issue for the scroll position in RadListBox //EHBSD-2359/EHBSD-1215/EHBSD-1412
    sender.saveClientState = function () {
        return "{" +
                    "\"isEnabled\":" + this._enabled +
                    ",\"logEntries\":" + this._logEntriesJson +
                   ",\"selectedIndices\":" + this._selectedIndicesJson +
                   ",\"checkedIndices\":" + this._checkedIndicesJson +
                   ",\"scrollPosition\":" + Math.floor(this._scrollPosition) +
               "}";
    }
    $('li', $('#' + sender.get_id())).each(function (index) {// for 508 and checkbox focus tooltip
        var item = $(this);
...
function updateExpandAndCollapseOptions() {
    ...
    var leftMenuDisabled = !$("#leftpanel").is(":visible");
    ...
    //TFS-57479 - add null check for layout menu to fix console error
    if (!layoutMenu) {
        return;
    }
...
function OpenPopup(strURL, lngHeight, lngWidth, strWindowName) {
    ...
    //start : Removing this logic to not append breadcrumb
    ...
    //    strURL += "?BreadCrumbROText=" + encodeURIComponent(breadcrumbText);
    //}
    //end : Removing this logic to not append breadcrumb
