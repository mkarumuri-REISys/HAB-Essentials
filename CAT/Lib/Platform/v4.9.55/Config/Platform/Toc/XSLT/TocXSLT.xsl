...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="showOptionsColumn"/>
	<xsl:template match="/">
        ...
        <table class="ehb_datatable floatTableHeader" summary="Table of Contents Information">
        
          <tr>
            <th class="rowheader" width="65%" scope="col">
              Section
            </th>
            <th class="rowheader datacenter" width="20%" scope="col">
              Type
            </th>
            <xsl:if test="$showOptionsColumn">
            <th  class="rowheader" width="10%" scope="col">
              Options
            </th>
            </xsl:if>
          </tr>
    ...
    <tr class="tocsectionHeader">     
      <xsl:element name="td">
        <xsl:attribute name="colspan">3</xsl:attribute>
        <xsl:attribute name="class">
          <xsl:if test="count(@customCss)=0 or @customCss = ''">rowSubheader</xsl:if>
          <xsl:if test="count(@customCss)>0 and @customCss != ''">
            <xsl:value-of select="@customCss"/>
          </xsl:if>
        </xsl:attribute>
        <xsl:if test="count(@displayValue)=0 or @displayValue = ''">&#160;</xsl:if>
        <xsl:if test="count(@displayValue)>0 and @displayValue != ''">
    ...
    <xsl:element name="tr">
      ...
      <xsl:if test="(position() mod 2 = 1)">
        <xsl:attribute name="class">colodd</xsl:attribute>
      </xsl:if>
      <xsl:if test="(position() mod 2 = 0)">
        <xsl:attribute name="class">coleven</xsl:attribute>
      </xsl:if>
				...
				<td class="datacenter"><xsl:value-of select="@type"/></td>
      <xsl:if test="$showOptionsColumn">
				<td >
                  ...
                  <xsl:attribute name="class">extIcon</xsl:attribute>
								</xsl:element>
							</xsl:when>
