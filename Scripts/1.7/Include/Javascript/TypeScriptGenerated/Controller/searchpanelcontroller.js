            ...
            var SearchPeristancePanelController = (function (_super) {
                ...
                SearchPeristancePanelController.prototype.collectSearchParameters = function () {
                    ...
                    for (var i = 0; i < searchParamControlsLength; i++) {
                        ...
                        // let's add the item to search list model (JSON)
                        this.updateSearchListModel(item);
                    }
                    //plsup
                ...
                // adds or udpates search list model item
                SearchPeristancePanelController.prototype.updateSearchListModel = function (item) {
                    var itemId = item.attr('id');
                    var value = "";
                    var itemId2, value2 = "";
                    if (item.hasClass("RadPicker")) {
                        var fromControl = $('#' + itemId).find('input');
                        itemId = fromControl.attr('id');
                        value = PlatformUtil.GetControlValue(itemId, true);
                        var toControl = $('#' + item.attr('id')).nextAll('.RadPicker').find('input');
                        if (toControl) {
                            itemId2 = toControl.attr('id');
                            value2 = PlatformUtil.GetControlValue(itemId2, true);
                        }
                    }
                    else {
                        if (itemId !== undefined) {
                            ...
                            var childItem = item.children().first();
                            itemId = childItem.attr('id');
                            value = PlatformUtil.GetControlValue(itemId, true);
                        }
                    }
                    var index = this.searchListModel.length;
                    var updated = false;
                    if (index > 0) {
                        for (var i = 0; i < index; i++) {
                            if (this.searchListModel[i].InputClientID === itemId) {
                                this.searchListModel[i].InputValue = value;
                                updated = true;
                            }
                            if (itemId2) {
                                updated = false;
                                if (this.searchListModel[i].InputClientID === itemId2) {
                                    this.searchListModel[i].InputValue = value2;
                                    updated = true;
                                }
                            }
                            if (updated)
                    ...
                    if (!updated) {
                        this.searchListModel.push(new UI.SearchFieldModel(itemId, value));
                        if (itemId2)
                            this.searchListModel.push(new UI.SearchFieldModel(itemId2, value2));
                    }
                };
                // adds or udpates search list model item
                SearchPeristancePanelController.prototype.updateSortListModel = function (fieldItem, sortItem) {
                    var idFieldControl = fieldItem.attr('id');
                    var valueField = PlatformUtil.GetControlValue(idFieldControl, false);
                    var idSortControl = sortItem.attr('id');
                    var valueSort = PlatformUtil.GetControlValue(idSortControl, false);
                    var index = this.sortListModel.length;
                    var updated = false;
                    if (index > 0) {
                        for (var i = 0; i < index; i++) {
                            if (this.sortListModel[i].InputClientID === idFieldControl) {
                                this.sortListModel[i].InputValue = valueField;
                                this.sortListModel[i].SortDirectionInputValue = valueSort;
                                updated = true;
                                break;
                            }
                        }
                    ...
                    if (!updated) {
                        this.sortListModel.push(new UI.SortFieldModel(idFieldControl, valueField, idSortControl, valueSort));
                    }
                };
                ...
                SearchPeristancePanelController.prototype.collectOrderBy = function () {
                    ...
                    var customSort = $('[id$=btnSearch]').attr('CustomSortSelected');
                    if (customSort !== undefined) {
                        if (customSort.toLowerCase() === 'true') {
                            ...
                            for (var i = 0; i < sortFilterControlsLength; i = i + 2) {
                                ...
                                //lets add this to sort list model 
                                this.updateSortListModel(itemFieldControl, itemSortControl);
                            }
                        }
                    }
                    return sortItems;
                ...
                SearchPeristancePanelController.prototype.collectSearchName = function () {
                    var name = "";
                    //PLSUP-5256
                    //If 'showsavedsearchlink' attribute is set to false on the searchpersistencepanel, then SearchNameBox1 will not exist and this will cause an error
                    //therefore we need to check if that element is there or not before referring to it.
                    if ($("[id*=SearchNameBox1]").length > 0) {
                        name = PlatformUtil.GetControlValue('SearchNameBox1', false).trim();
                        this.searchNameModel = new UI.SearchFieldModel('SearchNameBox1', name);
                    }
                    return name;
                };
                SearchPeristancePanelController.prototype.collectSearch = function () {
                    var searchModel = new UI.SearchModel(this.collectSearchParameters(), this.collectOrderBy());
                    //PLSUP-5256
                    //If 'showsavedsearchlink' attribute is set to false on the searchpersistencepanel, then SearchNameBox1 will not exist and this will cause an error
                    //therefore we need to check if that element is there or not before referring to it.
                    if (this.enablePersistence && $("[id*=SearchNameBox1]").length > 0) {
                        this.searchNameModel = new UI.SearchFieldModel('SearchNameBox1', PlatformUtil.GetControlValue('SearchNameBox1', false));
                        ReiSys.Platform.UI.SearchPersistUtility.saveState(this.searchListModel, this.sortListModel, this.searchNameModel, this.sortControlContainerId);
                    }
                    return searchModel;
                };
                ...
                SearchPeristancePanelController.prototype.clearSortItems = function () {
                    $('[id$=btnSearch]').attr('CustomSortSelected', 'false');
                    var sortFilterControls = $('div[id*=' + this.sortControlContainerId + '] div.RadComboBox');
                    var sortFilterControlsLength = sortFilterControls.length;
                    for (var i = 0; i < sortFilterControlsLength; i = i + 2) {
                        var itemFieldControl = $(sortFilterControls.get(i));
                        var idFieldControl = itemFieldControl.attr('id');
                        ReiSys.Utilities.Util.SetValueForControl(idFieldControl, '');
                        var itemSortControl = $(sortFilterControls.get(i + 1));
                        var idSortControl = itemSortControl.attr('id');
                        ReiSys.Utilities.Util.SetValueForControl(idSortControl, '');
                        //lets add this to sort list model 
                        this.updateSortListModel(itemFieldControl, itemSortControl);
                    }
                };
                SearchPeristancePanelController.prototype.getSavedSearch = function (menu) {
                    ...
                    if ($("[id*=SearchNameBox1]").length > 0) {
                        ...
                        var searchName = PlatformUtil.GetControlValue('SearchNameBox1', false);
                        if (menu.length > 0) {
                            var arrayOfObservableleItems = new Array();
                            for (var i = 0; i < menuItemCount; i++) {
                                var newItem = new UI.SavedSearchModelObservable(menu[i]);
                                if (newItem.SearchName() === searchName)
                ...
                SearchPeristancePanelController.prototype.populateControls = function (saveSearch) {
                    ...
                    //var searchValue = savedSearch.Value();
                ...
                SearchPeristancePanelController.prototype.getSavedSearchDetails = function (saveSearchId) {
                    ...
                    this.search(false);
                    //PLSUP-5727 : hide search panel when saved search is performed
                    this.hideSearchPanel(); //this.showSearchPanel();
                    var tiptip_holder = $("#tiptip_holder");
                    var tiptip_content = $("#tiptip_content");
                    var tiptip_arrow = $("#tiptip_arrow");
                    if (tiptip_content !== undefined && tiptip_content !== null) {
                        tiptip_holder.hide().removeAttr("class").css("margin", "0");
                        tiptip_arrow.removeAttr("style");
                        tiptip_content.hide();
                        tiptip_arrow.hide().removeAttr("style");
                    }
                };
                ...
                SearchPeristancePanelController.prototype.findSearchById = function (saveSearchId) {
                    ...
                    this.searchNameModel = new UI.SearchFieldModel('SearchNameBox1', searchItem.SearchName());
                    return searchItem;
                };
                SearchPeristancePanelController.prototype.clearSelected = function () {
                    ...
                    //TFS User Story 7132
                    //commented this line out and replaced it with the line below it because we do not want to remove the element, just hide it, so that we can show it again when needed
                    //$('[id*=SelItemImgDefault]').remove();
                    $('[id*=SelItemImgDefault]').hide();
                ...
                SearchPeristancePanelController.prototype.getDefaultSavedSearchModel = function () {
                    ...
                    //PLSUP-5260
                    //We were not checking to see if the last character is not a comma, in some cases it would be which is incorrect syntax for the JSON string and was causing an error
                    if (tmpValueItem.lastIndexOf(',') == tmpValueItem.length - 1)
                        tmpValueItem = tmpValueItem.substring(0, tmpValueItem.length - 1);
                    tmpValueItem += '}';
                ...
                SearchPeristancePanelController.prototype.executeDefaultSearch = function () {
                    //TFS User Story 7132
                    //changed so that 'Default Parameters' in the saved search menu is properly highlighted when it is selected 
                    this.clearSelected();
                    var defaultParamsCont = $("li[id *= defaultSearchParmContainer]");
                    var dplClass = defaultParamsCont.attr("class");
                    if (dplClass)
                        defaultParamsCont.attr("class", dplClass + " search_selected");
                    else
                        defaultParamsCont.attr("class", "search_selected");
                    $("img[id *= SelItemImgDefault]").show();
                    this.clearSearchItems();
                    this.clearSortItems();
                ...
                /**
                 * TFS-7495 : updates/removes custom sort selection on search panel control based on grid command filter events
                 */
                SearchPeristancePanelController.prototype.refreshCustomSortSelection = function () {
                    var grid = $telerik.findGrid(REISys.Platform.UI.SearchPeristancePanelController.radGridClientId);
                    var tableView = grid.get_masterTableView();
                    var sortItems = tableView.get_sortExpressions().toList();
                    var sortCount = sortItems.length;
                    var ctr = 0, customCtr = 0;
                    var clearAll = false;
                    // count previews custom sort items                                
                    var sortItem;
                    var i;
                    for (i = 0; i < this.sortListModel.length; i++) {
                        sortItem = this.sortListModel[i];
                        for (var j = 0; j < sortItems.length; j++) {
                            if (sortItems[j].FieldName === sortItem.InputValue) {
                                ctr++;
                                if (sortItem.SortDirectionInputValue === (sortItems[j].SortOrder === 2 ? "DESC" : "ASC"))
                                    customCtr++;
                            }
                        }
                    }
                    // if grid sort items is greater than the custom sort, clear all selections
                    if (sortCount > ctr || (sortCount === ctr && ctr > customCtr))
                        clearAll = true;
                    var hideCustomControls = clearAll;
                    for (i = 0; i < this.sortListModel.length; i++) {
                        sortItem = this.sortListModel[i];
                        var clearSelection = true;
                        for (var j = 0; j < sortItems.length; j++) {
                            var order = (sortItems[j].SortOrder === 2 ? "DESC" : "ASC");
                            var fieldName = sortItems[j].FieldName;
                            if (fieldName === sortItem.InputValue && order === sortItem.SortDirectionInputValue) {
                                clearSelection = false;
                                if (clearAll) {
                                    tableView._showSortIconForField(fieldName, Telerik.Web.UI.GridSortOrder.None);
                                    sortItems.splice(j, 1);
                                }
                                break;
                            }
                        }
                        if (clearSelection || clearAll)
                            $find(sortItem.InputClientID).clearSelection();
                        else
                            hideCustomControls = false;
                        if (hideCustomControls) {
                            $('[id$=btnSearch]').attr('CustomSortSelected', 'false');
                            var customSortSelectionIndicator = $($("input[id*=_CustomSortSelectionIndicator]")[0]);
                            customSortSelectionIndicator.val("false");
                            $($("div[id*=_contentDivImg3]")[1]).attr('style', 'display:none;');
                        }
                        else {
                            // need to set this back to true, since there is still remaining custom sort 
                            Reisys.Platform.UI.RadGrid.IsCustomSorted.set(true);
                        }
                    }
                };
                /**
                 * TFS-7495 : set grid sort expression based on custom sort expression
                 */
                SearchPeristancePanelController.prototype.displayCustomSortExpression = function () {
                    if (this.sortListModel !== undefined && this.sortListModel !== null && this.sortListModel.length > 0) {
                        var grid = $telerik.findGrid(REISys.Platform.UI.SearchPeristancePanelController.radGridClientId);
                        var tableView = grid.get_masterTableView();
                        //clear the previous sort expression if there is one
                        tableView.clearSort();
                        for (var i = 0; i < this.sortListModel.length; i++) {
                            var item = this.sortListModel[i];
                            if (item.InputValue === undefined || item.InputValue === null || item.InputValue === "_blank" || item.InputValue === "")
                                continue;
                            Reisys.Platform.UI.RadGrid.AddSortExpression(grid, item.InputValue, (item.SortDirectionInputValue === "DESC" ? 2 : 1));
                        }
                    }
                };
