...
function Menu(caller, options) {
    ...
    this.showMenu = function () {
        ...
        var firstItemlink = $('a:focusable', tempMenu).not('.cmDisabled').first();
                ...
                case 9: // tab
                    if (shiftTagHitPreviously) {
                        if ($(event.target).is('.' + options.linkHover)) {
                            ...
                            // To move to the prev link surpassing the div (non link) if the current link is not the last link.
                            // This addition is to fix the shift-tabbing issue.
                            if (prevLink.size() === 0 && $(event.target)[0] !== firstItemlink[0]) {
                                prevLink = $(event.target).parent().prev().prev().find('a:eq(0)');
                            }
                            if (prevLink.size() > 0) {
                                $(event.target).trigger('mouseout');
                                prevLink.trigger('mouseover');
                ...
                case 40: // down arrow 
                    if ($(event.target)[0] != lastItem[0]) {
                        if ($(event.target).is('.fg-menu-indicator')) {
                            ...
                            // To move to the next link surpassing the div (non link) if the current link is not the last link.
                            // This addition is to fix the tabbing issue.
                            if (nextLink.size() === 0 && $(event.target)[0] !== lastItem[0]) {
                                nextLink = $(event.target).parent().next().next().find('a:eq(0)');
                            }
                            if (nextLink.size() > 0) {
                                $(event.target).trigger('mouseout');
