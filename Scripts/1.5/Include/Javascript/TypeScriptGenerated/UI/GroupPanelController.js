    ...
    (function (Platform) {
        ...
        (function (UI) {
            ...
            var GroupPanelController = (function () {
                function GroupPanelController(args, masterPanel) {
                    ...
                    this.isMasterPanel = args.IsMasterPanel;
                    this.itemCountBtnId = args.ItemCountBtnId;
                    this.viewAllBtnId = args.ViewAllBtnId;
                    this.viewPopupBtnId = args.ViewPopupBtnId;
                    this.viewPopupBtnUrl = args.FinalPopupUrl;
                    this.useLocalStorage = args.UseLocalStorage;
                    if (this.isMasterPanel) {
                        this.grid = Reisys.Platform.UI.RadGrid.GetControllerInstance(args.GridId);
                        ...
                        this.localStorageKeyName = this.getLocalStorageKeyName(args.GridId);
                        //retreive from localStorage when initialized
                        this.items = this.useLocalStorage ?
                            this.getItemsFromLocalStorage(this.localStorageKeyName)
                            : new ReiSys.Platform.Utils.Dictionary();
                    }
                    else {
                ...
                GroupPanelController.prototype.addToCollection = function (item) {
                ...
                GroupPanelController.prototype.addItems = function (inputItems) {
                    ...
                    // create a new dictionary if null.
                    if (this.items === null)
                        this.items = new ReiSys.Platform.Utils.Dictionary();
                    for (var i = 0; i < len; i++) {
                        this.addToCollection(inputItems[i]);
                    }
                ...
                GroupPanelController.prototype.removeFromCollection = function (item) {
                    var key = this.generateKey(item);
                    if (this.items === null)
                        this.items = new ReiSys.Platform.Utils.Dictionary();
                    if (this.items.containsKey(key)) {
                ...
                GroupPanelController.prototype.removeItems = function (items) {
                    ...
                    for (var i = 0; i < len; i++) {
                        this.removeFromCollection(items[i]);
                    }
                    if (this.useLocalStorage) {
                        this.addToLocalStorage(this.localStorageKeyName, this.items);
                    }
                };
                GroupPanelController.prototype.clearGroupPanelSelection = function () {
                    if (this.useLocalStorage) {
