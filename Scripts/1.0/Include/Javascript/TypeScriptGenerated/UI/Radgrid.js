        ...
        (function (UI) {
            ...
            var RadGrid = (function () {
                ...
                RadGrid.prototype.GetData = function (parameterArgs) {
                    ...
                    //PLSUP-5268
                    //we are now checking to see if there are arguments from the OnBeforeServiceCalling event that tells us to not get the data to bind to the grid
                ...
                RadGrid.prototype.createSelectAllKeysItem = function (item) {
                    var gridDataKeyNamesLen = this.gridDataKeyNames.length;
                    var toReturn = {};
                    for (var j = 0; j < gridDataKeyNamesLen; j++) {
                ...
                RadGrid.UpdateSortAndFilterExpressionDisplay = function (grid) {
                    ...
                    for (i = 0, len = sortList.length; i < len; i++) {
                        ...
                        if ($("#clearAllExp") != null && $("#clearAllExp").children() != null && $("#clearAllExp").children().length > 0)
                            $("#clearAllExp").click();
                        sortItem = sortList[i];
                        sortModel = new GridSortModel(sortItem.FieldName, Reisys.Platform.UI.RadGrid.GetTitleByFieldName(grid, sortItem.FieldName, columnDefinitionModel), sortItem.SortOrder);
...
//# sourceMappingURL=Radgrid.js.map